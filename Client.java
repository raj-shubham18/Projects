...
//public class Client extends JFrame implements ActionListener   // we are making frame so Jframe is extended which class comes from swing package  
  ...
  Client() // constructor
    {
        ...
        // Profile picture(icon)
             ImageIcon i4 = new ImageIcon(ClassLoader.getSystemResource("icons/2.png"));  // setting the image on the frame(p1)
        //calling the Image class in which we get the the image of i1  and scaling(shapping) it . 
        ...
        f.setLocation(800,50);     // used to shift the frame from left side i.e X-axis() and from uppes side i.e Y-axis()
    ...
    public void actionPerformed (ActionEvent ae) //  overidden from the the interface as it have abstract class in it. 
    {
        ...
        dout.writeUTF(out);  // sending the message to the server
   ...
   public static void main(String[] args)
   {
       ...
       try {
           Socket s = new Socket("127.0.0.1",6001);  // creating a socket with local ip address and port no of the server and client shouls be same  to connect them
